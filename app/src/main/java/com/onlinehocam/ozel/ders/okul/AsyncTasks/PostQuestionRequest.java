...
public class PostQuestionRequest extends AsyncTask<String, Void, String> {
    ...
    public PostQuestionRequest(Activity activity, Context context, QuestionRequest questionRequest, LinearLayout linearLayoutMainProgressBar, LinearLayout linearLayoutButtonPostQuestionRequest, boolean isToDisplayProgressBarOnPreExecute, boolean isToDismissProgressBarOnPostExecute) {
        this.activity = activity;
        this.context = context;
        this.linearLayoutMainProgressBar = linearLayoutMainProgressBar;
        this.linearLayoutButtonPostQuestionRequest = linearLayoutButtonPostQuestionRequest;
    ...
    protected void onPostExecute(String result) {
        ...
        GlobalVariables.Log(context, "in PostQuestionRequest onPostExecute >>> resultString: "+resultString);

        int resultInt = -1;
        resultInt = Integer.parseInt(resultString);
        boolean isInsertSuccessful = (resultInt != -1);
        ...
        if(isInsertSuccessful)
        {
            if(activity instanceof StudentPostQuestionActivity)
            {
                ...
                GlobalVariables.AlertDialogDisplay(activity, context.getString(R.string.post_question_response_succesful));
                ((StudentPostQuestionActivity)activity).isQuestionRequestPostedSuccessfully = true;
                ((StudentPostQuestionActivity)activity).buttonPostQuestionRequest.setText(R.string.menu_main_manu);
            }
            else if(activity instanceof TutorSolutionPostActivity)
            {
                SoundHelper.PlayMediaPlayerSound(activity, SoundHelper.SOUNDS.PURCHASE);
                GlobalVariables.AlertDialogDisplay(activity, context.getString(R.string.post_question_response_succesful));
                ((TutorSolutionPostActivity)activity).isSolutionPostedSuccessfully = true;
                ((TutorSolutionPostActivity)activity).buttonPostQuestionRequest.setText(R.string.menu_main_manu);
            }
        }
        else
        {
